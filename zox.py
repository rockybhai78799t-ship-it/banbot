...
class AntiPremiumBot:
    ...
    # ---------- HTTP Helpers ----------
    async def make_get_request(self, url: str, params: dict = None) -> Optional[dict]:
        """Make GET request to FAM Gateway"""
        if not self.session:
    ...
    async def initiate_payment(self, user_id: int, chat_id: int, amount: float, days: int):
        """Initiate payment with FAM Gateway - EXACTLY like PHP version"""
        try:
            # Call FAM Gateway QR API
            result = await self.make_get_request(
                f"{BASE_URL}/api/qr.php",
                {
                    'api_key': FAM_API_KEY,
                    'amount': str(amount),
            ...
            if not result or result.get('status') != 'success':
                err_msg = result.get('message', 'Gateway connection timeout.') if result else 'Gateway connection timeout.'
                await self.app.send_message(chat_id, f"⚠️ Gateway Error: {err_msg}")
                return
            
            order_data = result['data']
            order_id = order_data['order_id']
            payable_amount = order_data.get('payable_amount', amount)
            checkout_url = f"{BASE_URL}/checkout.php?order_id={order_id}"
            
            # Store payment in DB
            self.db.add_payment(order_id, user_id, payable_amount, days)
            ...
            text = f"""💳 **Payment Session Initialized**
...
Complete the payment via checkout and click **Verify Payment**."""
    ...
    async def verify_payment(self, order_id: str, chat_id: int, callback_id: str = None):
        """Verify payment with FAM Gateway - EXACTLY like PHP version"""
        try:
            # Call FAM Gateway Verify API
            result = await self.make_get_request(
                f"{BASE_URL}/api/verify-order.php",
                {
                    'api_key': FAM_API_KEY,
                    'order_id': order_id
                }
...
⌨️ **Reply with the number of days you want (Minimum 7). Type `life` for lifetime access:**"""
        ...
        if data.startswith("verify_pay:"):
            ...
            # Don't send extra response, verify_payment handles it
            return
