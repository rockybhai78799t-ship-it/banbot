...
UPI: Chandaliya@fam
API Key: fam_76e80dd1401deb6f2e74aa34e270c6f41ff9b088
"""
...
class AntiPremiumBot:
    ...
    # ---------- Generate QR Code ----------
    def generate_upi_qr(self, amount: float) -> BytesIO:
    ...
    async def initiate_payment(self, user_id: int, chat_id: int, amount: float, days: int):
        """Initiate payment with FAM Gateway - UPDATED API with UPI"""
        try:
            url = f"{BASE_URL}/api/create-order.php"
            
            payload = {
                "amount": amount,
                "redirect_url": f"https://t.me/YourBotUsername",  # Change to your bot username
                "upi_id": UPI_ID,
                "payee_name": UPI_NAME,
                "payment_type": "upi"
            }
            ...
            # Generate QR Code
            qr_img = self.generate_upi_qr(amount)
...
**How to Pay:**
1. Scan QR Code below or use UPI
2. Pay to `{UPI_ID}`
3. Click **Verify Payment** after paying
4. Or click **Pay Online** for checkout page"""

            kb = InlineKeyboardMarkup([
                [InlineKeyboardButton("💳 Pay Online (Checkout)", url=checkout_url)],
    ...
    async def verify_payment(self, order_id: str, chat_id: int, callback_id: str = None):
        """Verify payment with FAM Gateway - UPDATED API"""
        try:
